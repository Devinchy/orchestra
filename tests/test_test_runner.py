...
# ---------- override por orchestra.toml del repo ----------

def test_read_repo_test_command_lee_orchestra_toml(tmp_path):
    (tmp_path / "orchestra.toml").write_text(
        '[tests]\ncommand = "pytest -k upload"\n', encoding="utf-8")
    assert tr.read_repo_test_command(tmp_path) == "pytest -k upload"


def test_read_repo_test_command_sin_archivo_es_none(tmp_path):
    assert tr.read_repo_test_command(tmp_path) is None


def test_read_repo_test_command_sin_seccion_tests_es_none(tmp_path):
    (tmp_path / "orchestra.toml").write_text('[otra]\nx = 1\n', encoding="utf-8")
    assert tr.read_repo_test_command(tmp_path) is None


def test_run_tests_orchestra_toml_gana_sobre_autodeteccion(tmp_path):
    # Hay pyproject (autodetectaría pytest) PERO orchestra.toml manda.
    (tmp_path / "pyproject.toml").write_text("[project]", encoding="utf-8")
    (tmp_path / "orchestra.toml").write_text(
        '[tests]\ncommand = "make test"\n', encoding="utf-8")
    calls = {}
